...
SELECT                    -- we use count() function to get total number
    COUNT(*)
FROM
    director_mapping;
...
FROM
    movie
WHERE                                -- we have used where clause to get null values
...
/* In Solution 2 above, id in each case statement has been used as a counter to count the number of null values. Whenever a value
   ...
   SELECT 
    COUNT(CASE                                      -- here we have used case statement to get output
...
SELECT 
    year, COUNT(*) AS number_of_movies            -- count() function 
FROM
    movie
GROUP BY year                                     -- group by clause
order by year;                                    -- order by clause
...
SELECT 
    MONTH(date_published) AS month_num,       -- we used month() function to get expected output
    date_published
FROM
    movie;
...
FROM 
    movie
WHERE 
    country IN ('USA', 'India')                       -- here we used 'in' operator
    AND year = (SELECT MAX(year) FROM movie);         -- here we used sub-query
...
FROM
    movie
WHERE country LIKE '%USA%';                    -- here we have used 'like' operator
...
SELECT 
    COUNT(*) AS number_of_movies              -- count() 
FROM 
    movie
WHERE                                         -- where clause
    (country LIKE '%USA%' OR country LIKE '%India%')       -- like operator 
    AND year = 2019;
...
SELECT DISTINCT                -- here we have distinct to get unique output
    genre
FROM
    genre;
...
SELECT 
    g.genre, COUNT(m.id) AS count            -- count()
FROM
    movie m
        JOIN                              -- we use 'join' to connect tables
    genre g ON g.movie_id = m.id
GROUP BY genre
ORDER BY count DESC
LIMIT 1;                             -- used limit clause to get limited output
...
WITH movie_genre_summary AS (                -- here we have used case statement 
    SELECT 
        movie_id,
...
SELECT 
    genre, AVG(m.duration) AS avg_duration          -- we avg() to get avg_duration
FROM
    genre g
        LEFT JOIN
...
with summary as                            -- we used cte to get expected output
	   (
        select genre,
        count(movie_id) as movie_count,
		rank() over(order by count(movie_id) desc) as genre_rank
...
FROM
 ...
 select 
      min(total_votes) as min_total_votes,               -- used min() to get minimum values
      max(total_votes) as max_total_votes                -- used max() to get minimum values
from ratings;
...
WITH top_movies AS (                   -- used cte
    SELECT
        m.title,
        AVG(r.avg_rating) AS avg_rating,
        ROW_NUMBER() OVER (ORDER BY AVG(r.avg_rating) DESC) AS movie_rank     -- here we applied row_number rank
    FROM
        movie m
    LEFT JOIN
...
SELECT 
    genre, COUNT(m.id) AS movie_count          -- used count() to get number of movies
FROM
    genre g
        JOIN
...
WHERE
    MONTH(m.date_published) = 3                  -- used month() to get month
        AND YEAR(m.date_published) = 2017        -- used year() to get year
        AND LOWER(m.country) = 'usa'             -- used lower() to get lower country names
...
SELECT 
    m.title, AVG(r.avg_rating) AS avg_rating, g.genre
FROM
    genre g
        JOIN
    ratings r USING (movie_id)
...
WHERE
    r.median_rating = 8
        AND m.date_published BETWEEN '2018-04-01' AND '2019-04-01';          -- here we used between operator
